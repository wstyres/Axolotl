...
static void gdt_set_entry(gdt_entry_t* entry, uint32_t base, uint32_t limit, uint16_t flag) {
...
void gdt_table_initialize() {
    ...
    gdt_set_entry(&entries[0], 0, 0, 0);
    gdt_set_entry(&entries[1], 0x00000000, 0x000FFFFF, (GDT_CODE_PL0));
    gdt_set_entry(&entries[2], 0x00000000, 0x000FFFFF, (GDT_DATA_PL0));
    gdt_set_entry(&entries[3], 0x00000000, 0x000FFFFF, (GDT_CODE_PL3));
    gdt_set_entry(&entries[4], 0x00000000, 0x000FFFFF, (GDT_DATA_PL3));

    reloadGDT(&descriptor);
}